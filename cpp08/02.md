...
## Estructura de archivos: .hpp y .tpp

`MutantStack` es una clase **completamente template** — todos sus metodos dependen de `T`. Por eso no hay `MutantStack.cpp`. En su lugar se usa el patron `.tpp`:

- `MutantStack.hpp` → declaraciones de la clase (include guards, typedef, firmas de metodos)
- `MutantStack.tpp` → implementaciones de los metodos template
- Al final de `MutantStack.hpp` se incluye el `.tpp`:

```cpp
// Al final de MutantStack.hpp:
#include "MutantStack.tpp"
```

Esto hace que el compilador vea la implementacion completa siempre que incluya el header, igual que si todo estuviera en el `.hpp`. El `.tpp` es una convencion visual para separar declaracion de implementacion sin mezclarlas en el mismo bloque.
