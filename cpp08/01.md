...
## Añadir con rango de iteradores
...
En lugar de llamar `addNumber` miles de veces. Internamente usa `_numbers.insert()` para insertar todo el rango de golpe, comprobando primero que no se supera la capacidad.

### Por que addNumbers va en el .hpp y no en el .cpp

`addNumbers` es una **funcion template** (el tipo `InputIterator` es generico). Al igual que `easyfind` en ex00, las funciones template deben estar completamente visibles en el momento de la instanciacion:

- `addNumber`, `shortestSpan`, `longestSpan` → van en `Span.cpp` (son funciones normales)
- `addNumbers` → va en `Span.hpp`, dentro de la definicion de la clase (es template)

Si intentaras mover `addNumbers` a `Span.cpp`, el compilador daria un error de enlazado porque no podria generar el codigo para cada tipo de iterador concreto que uses.
...
|------------|----------------|
| `std::sort` | Ordenar la copia del vector en shortestSpan |
| `std::adjacent_difference` | Calcular diferencias entre elementos consecutivos en shortestSpan |
| `std::min_element` | Encontrar la minima diferencia (shortestSpan) y el minimo valor (longestSpan) |
| `std::max_element` | Encontrar el maximo valor para longestSpan |
| `std::vector::insert` | Insertar un rango entero de iteradores en addNumbers |
| `std::distance` | Calcular cuantos elementos tiene el rango antes de insertarlos |
| `std::vector::push_back` | Añadir un unico elemento en addNumber |
| `std::vector::size` | Comprobar si el Span esta lleno |
